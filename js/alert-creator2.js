$( document ).ready(function() {
  const defaultHTML = `<div id="alert-image-wrap">
  ...
  const defaultCSS = `.widget-AlertBox {
    position: relative;
  }
  body,
  html {
  ...
  $(".expand-code").on("click", function( event ) {
    ...
    let addCodeTab = `<div class="code-tab"><div class="entry entry-code"><i class="fa-solid fa-plus"></i></div></div>`;
  ...
  $(".setting-row").on("click", ".entry.entry-remove-code", function() {
    ...
    let codeType = $(this).parents(".code-header").find(".title").html().toLowerCase();
    let codeName = $(this).siblings(".code-button").attr("data-type");
    let codeInput = codeName + "_custom_" + codeType +  "_input";
    $(this).parents(".code-setting").find("textarea").each(function() {
      if ($(this).attr("id") == codeInput) {
        $(this).remove();
      }
    });
    if ( $(this).parents(".code-setting").find("textarea").length === 1) {
      $(this).parents(".code-setting").find("textarea").first().show();
    }
  ...
  $(".code-tabs").on("click", ".entry.entry-code", function() {
    const options = {"default": "Alert Type", "donation": "SL Donations", "merch": "SL Merch", "loyalty_store_redemption": "SL Cloudbot Redemption", "prime_sub_gift": "SL Prime Gift", "streamlabscharitydonation": "SL Charity Donation", "follow": "Twitch Follows", "sub": "Twitch Subs", "host": "Twitch Hosts", "bits": "Twitch Bits", "raid": "Twitch Raids", "subscriber": "YT Subs", "sponsor": "YT Members", "fanfunding": "YT Super Chats", "facebook_follow": "FB Follows", "facebook_stars": "FB Stars", "facebook_like": "FB Likes", "facebook_support": "FB Supports", "facebook_support_gifter": "FB Support Gifters", "facebook_share": "FB Shares", "trovo_follow": "Trovo Follows", "trovo_sub": "Trovo Subs", "trovo_raid": "Trovo Raids", "pledge": "Pledges", "sponsored_campaign": "Sponsored Campaign", "donordrivedonation": "Charity Streaming Donations", "eldonation": "Extra Life Donations", "tiltifydonation": "Tiltify Donations", "treat": "Treats", "justgivingdonation": "JustGiving", "gamewispsubscription": "Game Wisp"};
    ...
    let codeTab = `<div class="code-tab"><div class="code-title" id="` + keys[0] + `"><div class="entry entry-remove-code"><i class="fa-solid fa-xmark"></i></div><select class="code-title-select">` + optionsArray + `</select></div></div>`;
    let addCodeTab = `<div class="code-tab"><div class="entry entry-code"><i class="fa-solid fa-plus"></i></div></div>`;
...
function saveAlertBox() {
  ...
  if (fetchCheck(saveText)) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      // dont set as null, pull from mainsetting?
      if(key.includes("_custom_html") && !key.includes("_enabled")) {
        let newHTML1 = allSettings[key];
        if (!newHTML1) {
          allSettings[key] = null;
        } else {
          let newHTMLKey = newHTML1.replace(/\"/g, '~3qt~').replace(/\n/g, "\\n").replace(/\t/g, "\\t");
          allSettings[key] = newHTMLKey;
        }
        //console.log(`key: ${key}, value: ${allSettings[key]}`);
      }

      if(key.includes("_custom_js") && !key.includes("_json")) {
        let newJS1 = allSettings[key];
        if (!newJS1) {
          allSettings[key] = null;
        } else {
          let newJSKey = newJS1.replace(/"/g, '~3qt~').replace(/\n/g, '\\n');
          allSettings[key] = newJSKey;
        }
      }

      if(key.includes("_custom_json")) {
        let newJSON = allSettings[key];
        if (!newJSON) {
          allSettings[key] = null;
        } else {
      ...
      if(key.includes("_custom_css")) {
        let newCSS1 = allSettings[key];
        if (!newCSS1) {
          allSettings[key] = null;
        } else {
          let newCSSKey = newCSS1.replace(/\n/g, "\\n").replace(/\t/g, "\\t");
